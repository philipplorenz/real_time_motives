...
retweets <- retweets %>% select(contains("retweet_motive"), expression_identity, informing_persuasion, socializing_attention, provokation, expressopinion, informothers, bringattention, connectwothers, showemotions, showattitude, entertain,
                                influence, provepoint, gainattention, savecontent, surpriseothers, provoke, showachievement, causechaos, deceiveothers,
                                user, following_retweeter, age_num, political, employment, gender, outrage, outraging, log_followers, log_retweets, education,
...
for (dep_var in dependent_variables) {
  ...
  model2 <- brm(as.formula(paste0(dep_var, " ~ outrage + category + sentiment_bin + following_retweeter + url_type + emotion_top + (1 | user)")), retweets)
...
for (dep_var in dependent_variables) {
  ...
  avg_comp_df$label <- paste(avg_comp_df$contrast)
